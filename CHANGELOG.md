...
- **Receipt script cache mismatch** - booking, admin, host, and login pages now load a fresh version of the receipt client so browsers cannot combine the new File payload with the old JSON-only uploader.
- **Embedded-browser upload fallback** - receipt uploads no longer rely on cross-realm `instanceof Blob` checks and retry as Base64 only when the server explicitly reports a dropped multipart image.
- **Early receipt evidence checkpoint** - the server stores and attaches a hash-addressed receipt before pricing, perceptual hashing, or OCR, so a later timeout cannot leave a paid booking without proof.
- **Mobile image normalization** - large phone screenshots are resized and encoded as a browser-safe JPEG before upload, reducing WebView memory and transfer failures while leaving small supported images unchanged.
